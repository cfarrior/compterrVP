...
pcompo.lab = "degree of overproliferation"
...
{#Supplement figure B1. Model 1: effect of cx, given n = 3
	...
	for(column in c(2,1,3,13)){ #(2) x, (1) r, (3) fESS, (13) fraction of reproduction lost due to belowground competition
		...
		mtext("cx, cost of sending \nroots away",side=1,line=3,cex=.8)
...
{#Figure 4, Figure B2,B3,B4. Model 2. Predictions across environmental gradients, under various assumptions of the cost of moving roots through soil (cxnot) and leakage (delta). 
	...
	for(deltaD in c(0,0.02)){ #default values of delta
		for(cxnotD in c(0,1.2)){ #default values of cxnot
			...
			for(var in c("R","cx","del")){
				if(var=="R") {data = Rvar; x =data$R; xlab=expression(paste(italic(R),", resource input (g ",dm^-2,yr^-1,")"));letter="A"}
				...
				if(var=="del") mtext(expression(paste(italic(r))),side=4,line=2.5,cex=.75)
...
}#end Figure 4, Figure B2,B3,B4


{#Figure B5. Comparison of the ESS and population-level optimum of allocation to roots. Qualitative patterns are the same for r and r*nd as plotted. 
	...
	for(var in c("R","cx","del")){
		...
		if(var=="R") mtext(expression(paste("fine root biomass (g)")),side=2,line=2)
		axis(side=1,cex=.5)
		axis(side=2,cex=.5)
		
		if(var=="R") legend(0.5,1.5,legend=c(expression(paste(italic(r),scriptstyle(ESS),"*",italic(n),scriptstyle(d),sep="")),expression(paste(italic(r),scriptstyle(OPTIM),"*",italic(n),scriptstyle(d),sep=""))),bty="n",col=c(cols[2],cols[3]),lwd=c(1,1),lty=c(1,1),cex=1)
